...
async function refreshWhenSettled(tries=40){
  for(let i=0;i<tries;i++){
    await refresh();
    const st=state.data?.run?.state;
...
async function loadCorpusProjects(){
  ...
  /* The first call reads every input PDF to detect its tier. Saying so beats
     an empty panel the presenter has to explain. */
  host.innerHTML=`<div class="section-heading mt"><h2>Reference projects</h2>${tag('Reading the corpus…')}</div><div class="loading"><span class="spinner"></span>Checking which playbook reads each drawing set…</div>`;
  try{
...
/* Recorded runs. Offered as what they are — a previously verified run, served
   read-only — and never opened silently in place of a live one. If the room
   is looking at a recording, the room is told. */
async function loadRecorded(){
  const host=$('#recorded-runs'); if(!host)return;
  try{
    const runs=await api('/fallback');
    if(!runs.length)return;
    host.innerHTML=`<div class="section-heading mt"><h2>Recorded runs</h2>${tag('Read-only fallback','amber')}</div>`+
      runs.map(r=>`<button class="scenario-option" onclick="openRecorded(${JSON.stringify(r.run_id).replace(/"/g,'&quot;')})">
        <strong>${esc(r.label||r.project_id)} ${ic('arrowRight')}</strong>
        <span>Captured ${esc((r.captured_at||'').replace('T',' ').replace('+00:00',' UTC'))} · ${fmt(r.released_mass_kg,1)} kg released · ${r.verified_against_reference?'reconciled '+esc(r.reference_lines_matched)+' with the reference':'not reconciled with a reference'}</span>
      </button>`).join('')+
      `<div class="notice">Use these only if the live path is unavailable. Clarifications cannot be answered against a recorded run — the numbers are fixed at capture.</div>`;
  }catch{/* a missing fallback store is not an error worth showing a client */}
}
async function openRecorded(runId){
  $('#modal').close();
  $('#main').innerHTML='<div class="loading"><span class="spinner"></span>Loading the recorded run…</div>';
  try{
    state.runId=runId;localStorage.setItem('trustsight-run',runId);
    state.scene=null;state.page=1;state.selectedItem=0;
    await refresh();
    state.pages=(await api(`/runs/${runId}/document`)).pages;
    go('overview');
...
/* Facts panel. The element family decides what is worth showing: a footing
   has no diameter and no spiral, and printing "No supported pile facts found"
   over a footing that was read correctly makes a working read look broken. */
function factRows(){
  const e=state.data.elements[0]; if(!e)return [];
  const p=e.geometry?.params||{};
  const family=e.element_type||'element';
  const label=family.replaceAll('_',' ');
  const cap=label.charAt(0).toUpperCase()+label.slice(1);
  const rows=[['count',`${cap} instances`,e.instances,'Instance basis',
    `Each ${label} multiplies the per-element bar count.`]];

  if(p.diameter!=null){
    rows.push(['diameter',`${cap} diameter`,`${fmt(p.diameter)} mm`,'Schedule / section',
      'Geometry used for the spatial completeness view.']);
  }else if(p.width!=null){
    rows.push(['diameter','Plan size',
      `${fmt(p.width)} × ${fmt(p.depth ?? p.width)} mm`,'Foundation sketch',
      'Plan dimensions of the foundation, used for the spatial view.']);
  }
  if(p.length!=null){
    rows.push(['length',p.diameter!=null?`${cap} length`:'Thickness',
      `${fmt(p.length)} mm`,'Source drawing',
      p.diameter!=null
        ? 'Overall length is distinct from an approved reinforcement run length.'
        : 'Slab thickness. It does not by itself give a bar cutting length.']);
  }

  /* one row per reinforcement entry, whatever the role happens to be */
  for(const r of e.reinforcement||[]){
    const role=String(r.role||'reinforcement').replaceAll('_',' ');
    const roleCap=role.charAt(0).toUpperCase()+role.slice(1);
    if(r.count!=null){
      rows.push([`bars-${r.role}`,`${roleCap} bars`,`${r.count} × ${r.bar_size}`,
        'Reinforcement callout',
        'An explicit count gives bars per element. The cutting length still needs leg dimensions.']);
    }
    if(r.spacing_mm){
      rows.push([`spacing-${r.role}`,`${roleCap} spacing`,
        `${r.bar_size} @ ${r.spacing_mm} mm`,'Reinforcement detail',
        'A spacing alone cannot determine quantity without the reinforced run length.']);
    }
    if(r.cover_mm!=null){
      rows.push([`cover-${r.role}`,`${roleCap} cover`,`${fmt(r.cover_mm)} mm`,
        'Stated on the drawing',
        'Cover stated on the drawing rather than taken from the rulebook.']);
    }
  }
  return rows;
}
...
function review(){const d=state.data,conflicts=d.elements.flatMap(e=>e.conflicts),awaitingApproval=d.run.state==='suspended'&&d.run.pending_step===19;$('#main').innerHTML=head('Resolve uncertainty. Keep the evidence.','Every clarification has a value, an owner and a reason.',button(`View schedule ${ic('arrowRight')}`,"go('results')",'outline'))+`<div class="review-layout"><div>${conflicts.length?`<div class="notice red gap"><b>Element-wide release blocked</b><br>${conflicts.map(esc).join('<br>')}<br>Answering reinforcement questions does not remove this conflict. Start a new clarification scenario to explore a complete release.</div>`:''}${d.questions.length?d.questions.map((q,i)=>questionCard(q,i)).join(''):awaitingApproval?'':`<div class="panel empty"><span style="color:var(--ok)">${ic('check',34)}</span><strong>${conflicts.length?'No missing-field questions':'Clarifications complete'}</strong>${conflicts.length?'The conflicting count still prevents release.':'Approved answers have been applied and the schedule recalculated.'}<p>${button(`Inspect calculated results ${ic('arrowRight')}`,"go('results')")}</p></div>`}${awaitingApproval?approvalGateCard():''}${d.schedule.exceptions?.length?`<details class="panel panel-pad mt"><summary style="font-size:12px">Calculation exceptions (${d.schedule.exceptions.length})</summary>${d.schedule.exceptions.map(x=>`<p class="explanation">${esc(x)}</p>`).join('')}</details>`:''}${d.unresolved.length?`<div class="notice mt">${d.unresolved.map(esc).join('<br>')}</div>`:''}</div><div><div class="panel panel-pad"><div class="eyebrow">LIVE RELEASE IMPACT</div><div class="impact-pair"><div><strong>${fmt(d.released_mass_kg,1)}</strong><small>kg released${d.seeded?' in sample':''}</small></div><div><strong>${d.questions.length}</strong><small>question${d.questions.length===1?'':'s'} still open</small></div></div><p class="explanation mt">An answer triggers a new deterministic calculation. Only supported items progress. The original evidence stays in the history.</p></div><div class="panel panel-pad mt"><div class="section-heading"><h2>Decision history</h2>${tag(d.history.length+' answers','green')}</div>${d.history.length?d.history.map(h=>`<div class="history-record"><b>${esc(h.label||h.subject||'decision')}</b>${h.kind&&h.kind!=='clarification'?tag(h.kind,'amber'):''}<p>${esc(h.detail||'')}</p><p>${esc(h.approver)} · ${new Date(h.timestamp).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})}</p><p>${esc(h.rationale)}</p></div>`).join(''):'<p class="explanation">No answers yet. Your first decision will appear here with its rationale and timestamp.</p>'}</div></div></div>`;}
/* The gate sits UNDER the open clarifications, not above them.
   Rendered first, a reader works top-down, clicks the one blue button on the
   screen, and signs a rulebook for quantities that do not exist yet —
   releasing nothing and making the product look broken at the exact moment
   it is meant to look careful. While anything is unanswered the card says so
   and offers no form; once the questions are gone it becomes the last step. */
function approvalGateCard(){
  const d=state.data, open=d.questions.length;
  if(open){
    return `<div class="question-card approval-card is-waiting">
      <div style="display:flex;justify-content:space-between">${tag('APPROVAL GATE')}${tag('rulebook')}</div>
      <h3>The signature comes last</h3>
      <p>${open} fact${open===1?' is':'s are'} still unstated, so there is nothing yet for a signature to release. Answer the clarification${open===1?'':'s'} above and this becomes the final step.</p>
      <p class="explanation mt">${esc(d.rulebook?.version||'This rulebook')} is unsigned. On a real project an unsigned assumption sheet holds every quantity back the same way.</p>
    </div>`;
  }
  return `<div class="question-card approval-card">
    <div style="display:flex;justify-content:space-between">${tag('APPROVAL GATE','red')}${tag('rulebook')}</div>
    <h3>Sign off the rulebook before anything releases</h3>
    <p>${esc(d.rulebook?.version||'This rulebook')} has not been approved by an engineer. Every quantity is calculated — the release gate is the only thing withheld, exactly as it would be on a real project with an unsigned assumption sheet.</p>
    <form id="approval-form" onsubmit="submitApproval(event)">
      <div class="input-grid">
        <div class="full"><label class="field">Approved by</label><input name="approver" required placeholder="Engineer name or email"></div>
        <div class="full"><label class="field">Approval rationale</label><textarea name="rationale" required placeholder="Basis for signing off this rulebook"></textarea></div>
      </div>
      <div class="form-actions">${d.seeded?`<button type="button" class="quiet" onclick="sampleApproval()">${ic('wand',13)} Fill sample answer</button>`:'<span></span>'}<button class="button" type="submit">Approve rulebook &amp; release ${ic('arrowRight')}</button></div>
      <div class="error" role="alert"></div>
    </form>
  </div>`;
}
...
/* ── Value & accuracy (spec s12) ────────────────────────────────────────────
   The commercial screen, and the easiest one in the product to make dishonest.
   Three rules hold it together:
     · nothing here is a constant. Every figure comes from this run.
     · with no estimator baseline, no saving is shown at all — not a default,
       not an industry average, not a hedged range.
     · recall is printed above variance. A line the engine never produced is a
       safety problem; two bars of difference is a commercial one, and an
       average of the two hides the first inside the second. */
const pct = v => v==null?'—':(v*100).toFixed(1)+'%';
/* A seeded run finishes in under a millisecond, and "0.00 s" beside "0 ms
   across 14 executed steps" reads as a broken instrument rather than a fast
   pipeline. Show the unit that carries the measurement: sub-second values in
   ms, and a genuine zero as the honest "under 1 ms" rather than "0". */
const secs = v => v==null?'—'
  :v<1?(v*1000<1?'&lt;1 <em>ms</em>':Math.round(v*1000)+' <em>ms</em>')
  :v<60?v.toFixed(1)+' <em>s</em>'
  :(v/60).toFixed(1)+' <em>min</em>';
function savingPanel(r){const s=r.saving;
  if(!s.available)return `<div class="panel panel-pad"><div class="section-heading"><h2>Time saved</h2>${tag('Not claimed','amber')}</div>
    <p class="explanation">${esc(s.reason)}</p>
    ${state.data?.recorded?'<p class="explanation mt">A recorded run cannot take a new baseline — its figures are fixed at capture. Start a live run to measure against your own.</p>':`<label class="field mt">Estimator's manual time for this scope (minutes)</label>
    <div style="display:flex;gap:8px;align-items:center"><input id="baseline-input" type="number" min="1" max="10000" step="1" placeholder="e.g. 45" style="max-width:160px">${button('Record baseline','setBaseline()','outline small')}</div>
    <p class="explanation mt">It is recorded against the run and every figure below recomputes from measured time. Nothing is stored client-side and nothing is assumed.</p>`}</div>`;
  const good=s.minutes_saved>0;
  const trust=s.review_time_representative!==false;
  return `<div class="panel panel-pad"><div class="section-heading"><h2>Time saved</h2>${tag(trust?'Measured this run':'Machine time only',trust&&good?'green':'amber')}</div>
    ${trust?'':`<div class="notice amber" style="margin-bottom:var(--s-3)"><b>Not a productivity figure.</b> ${esc(s.caveat)}</div>`}
    <div class="equation"><small>Estimator baseline for this scope — stated, not assumed</small><b>${fmt(s.manual_baseline_minutes)} minutes</b></div>
    <div class="equation"><small>This run — machine ${secs(r.effort.machine_seconds)} + review ${secs(r.effort.human_seconds)}</small><b>${s.elapsed_minutes.toFixed(2)} minutes</b></div>
    <div class="equation"><small>Difference — covers ${esc(s.covers||'this run')}</small><b>${Math.abs(s.minutes_saved).toFixed(2)} minutes ${good?'saved':'longer'} · ${fmt(Math.abs(s.pct_saved),1)}%</b></div>
    ${s.complete===false?`<div class="notice amber mt"><b>Partial scope.</b> ${esc(s.partial_note)}</div>`:''}
    <p class="explanation mt">${trust?esc(s.caveat):''} ${state.data?.recorded?'':button('Change baseline','clearBaseline()','outline small')}</p></div>`;}
async function setBaseline(){const v=Number($('#baseline-input')?.value);if(!v||v<1)return toast('Enter the baseline in whole minutes.');try{await post(`/runs/${state.runId}/baseline`,{manual_baseline_minutes:Math.round(v)});await refresh();value();toast('Baseline recorded. Saving is now measured against it.');}catch(e){toast(e.message);}}
async function clearBaseline(){try{await post(`/runs/${state.runId}/baseline`,{manual_baseline_minutes:null});await refresh();value();}catch(e){toast(e.message);}}
function claimsBar(c){const seg=[['released',c.released,'var(--ok)'],['review',c.review,'var(--attn)'],['blocked',c.blocked,'var(--stop)'],['unresolved',c.unresolved,'var(--muted)']].filter(s=>s[1]>0);
  if(!c.total)return '<p class="explanation">No claims yet.</p>';
  return `<div class="claims-bar">${seg.map(([k,n,col])=>`<i style="flex:${n};background:${col}" title="${n} ${k}"></i>`).join('')}</div>
    <div class="claims-key">${seg.map(([k,n])=>`<span>${n} ${esc(k)}</span>`).join('')}</div>`;}
function accuracyPanel(a){
  if(!a)return `<div class="panel panel-pad"><div class="section-heading"><h2>Accuracy</h2>${tag('No reference')}</div><p class="explanation">This project has no independently prepared bar list, so no accuracy is reported. Uploading one enables recall and variance without it ever reaching the extraction path.</p></div>`;
  const clean=a.element_recall===1&&a.mass_variance_kg===0;
  return `<div class="panel panel-pad"><div class="section-heading"><h2>Accuracy against the reference</h2>${tag(clean?'Exact':'Variance present',clean?'green':'amber')}</div>
  <p class="explanation">${esc(a.label||'Reference bar list')} — read only after generation, never by the extraction path.</p>
  <div class="metrics mt">
    ${metric('LINE RECALL',pct(a.element_recall),`${a.lines_generated} of ${a.reference_lines} reference lines produced`,a.element_recall===1?'green':'amber')}
    ${metric('EXACT LINES',pct(a.exact_rate),`${a.lines_exact} match quantity and length`,a.exact_rate===1?'green':'amber')}
    ${metric('MASS VARIANCE',(a.mass_variance_kg>0?'+':'')+fmt(a.mass_variance_kg,1)+' <em>kg</em>',`${fmt(a.generated_mass_kg,1)} generated vs ${fmt(a.reference_mass_kg,1)} reference`,a.mass_variance_kg===0?'green':'amber')}
  </div>
  <div class="table-wrap mt"><table><thead><tr><th>Mark</th><th>Size</th><th>Reference qty</th><th>Generated qty</th><th>Qty variance</th><th>Length variance</th><th>Outcome</th></tr></thead><tbody>
    ${a.lines.map(l=>`<tr><td><b>${esc(l.mark||'—')}</b></td><td>${esc(l.size||'—')}</td><td>${fmt(l.reference_quantity)}</td><td>${l.generated_quantity==null?'—':fmt(l.generated_quantity)}</td><td>${l.quantity_variance==null?'—':(l.quantity_variance>0?'+':'')+l.quantity_variance}</td><td>${l.length_variance_mm==null?'—':(l.length_variance_mm>0?'+':'')+fmt(l.length_variance_mm)+' mm'}</td><td>${tag(l.outcome.replaceAll('_',' '),l.outcome==='exact'?'green':l.outcome==='not_generated'?'red':'amber')}</td></tr>`).join('')}
  </tbody></table></div></div>`;}
function value(){const d=state.data,r=d.roi;
  if(!r){$('#main').innerHTML=head('Value & accuracy','Measured telemetry is not available for this run.','')+'<div class="empty"><strong>No ROI payload</strong>Start a run to record machine time, review time and accuracy.</div>';return;}
  const c=r.claims,w=r.work;
  $('#main').innerHTML=head('What this run actually cost, and what it got right.',
    'Every figure on this page is measured from this run. No industry averages, no assumed baselines, no rounded-up percentages.',
    `<button class="button outline" onclick="openExports()">${ic('download',12)} Download pack</button>`)+`
  <div class="metrics">
    ${metric('MACHINE TIME',secs(r.effort.machine_seconds),`${fmt(r.effort.machine_ms)} ms across ${r.steps.length} executed steps`)}
    ${metric('REVIEW TIME',r.effort.human_seconds?secs(r.effort.human_seconds):'None <em>yet</em>',r.effort.human_seconds?'Question raised to answer submitted, this session':'No question has been answered in this session')}
    ${metric('CLAIMS RELEASED',`${c.released} <em>/ ${c.total}</em>`,c.needing_human?`${c.needing_human} still need a person`:'Every claim reached a releasable number',c.needing_human?'amber':'green')}
    ${metric('KNOWLEDGE REUSED',w.knowledge_reused,'Clarifications avoided by prior approved facts')}
  </div>
  <div class="grid-two mt">${savingPanel(r)}
    <div class="panel panel-pad"><div class="section-heading"><h2>Where the work went</h2>${tag('Per claim, not per document')}</div>
      ${claimsBar(c)}
      <p class="explanation mt">Counted per claim because one drawing routinely carries a released bar and a blocked one at the same time. A document-level percentage would report this pile as finished while its spiral is still unanswered.</p>
      <div class="equation mt"><small>Values transcribed from the drawing</small><b>${fmt(w.values_extracted)}</b></div>
      <div class="equation"><small>Routed to a person — exceptions raised</small><b>${fmt(w.exceptions)}</b></div>
      <div class="equation"><small>Human decisions recorded, with rationale</small><b>${fmt(w.human_decisions)}</b></div>
      <div class="equation"><small>Iteration — same run id, ${r.iteration===1?'first':'continued'} pass</small><b>${r.iteration}</b></div>
    </div></div>
  <div class="section-space">${accuracyPanel(r.accuracy)}</div>
  <div class="section-space"><div class="section-heading"><h2>Step-by-step telemetry</h2>${tag(r.steps.length+' executed · route '+esc(r.route||'—'))}</div>
  <div class="panel table-wrap"><table><thead><tr><th>#</th><th>Step</th><th>Duration</th><th>Items</th><th>Values extracted</th><th>Exceptions</th><th>Knowledge reused</th></tr></thead><tbody>
    ${r.steps.map(s=>`<tr><td class="mono muted">${s.step}</td><td>${esc(s.name.replaceAll('_',' '))}</td><td>${fmt(s.duration_ms)} ms</td><td>${fmt(s.pages_or_items_processed)}</td><td>${fmt(s.values_extracted)}</td><td>${s.exceptions_raised?tag(s.exceptions_raised,'amber'):'0'}</td><td>${fmt(s.reused_knowledge_count)}</td></tr>`).join('')}
  </tbody></table></div>
  <div class="notice mt">Machine time here is computation only. It excludes the estimator's review, which is measured separately above — combining them would flatter the machine.</div></div>`;}
...
const RELEASE_STYLE={released:{c:'#7ccbb1',w:1.2,dash:0},review:{c:'#e8c17c',w:1,dash:6},blocked:{c:'#e8897f',w:1,dash:4},pending:{c:'#8fa6bd',w:.9,dash:3}};
      elements.forEach(({node,p},idx)=>{const L=node.params.length||0,x=p.x||0,z=p.y||0;if(!L)return;
      const style=RELEASE_STYLE[node.release_state]||RELEASE_STYLE[node.state==='conflicted'?'blocked':node.state==='ok'?'released':'pending'];
      const color=style.c;
      const r=(node.params.diameter||0)/2;
      if(r){
        /* cylindrical envelope: piles */
        for(const y of [0,-L]){for(let i=0;i<48;i++){let a=i*Math.PI/24,b=(i+1)*Math.PI/24;line([x+r*Math.cos(a),y,z+r*Math.sin(a)],[x+r*Math.cos(b),y,z+r*Math.sin(b)],color,style.w,style.dash);}}
        for(let i=0;i<8;i++){const a=i*Math.PI/4;line([x+r*Math.cos(a),0,z+r*Math.sin(a)],[x+r*Math.cos(a),-L,z+r*Math.sin(a)],color+'80',.75,style.dash);}
      }else{
        /* rectangular envelope: pad footings and anything else with a plan size.
           Without this branch a correctly read footing draws nothing at all,
           which looks like a failed read rather than a different shape. */
        const w=(node.params.width||0)/2, d=(node.params.depth||node.params.width||0)/2;
        if(!w||!d)return;
